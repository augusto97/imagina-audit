...
export default function PluginsSection({ report }: { report: SnapshotReport }) {
  ...
  return (
    <SectionCard
      title="Plugins"
      subtitle={`${s.total} instalados · ${s.active} activos · ${s.outdated} desactualizados · ${s.vulnerable} con vulnerabilidades conocidas`}
      icon={<Puzzle className="h-4 w-4 text-[var(--accent-primary)]" strokeWidth={1.75} />}
    >
      {/* KPIs */}
      <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <KpiTile label="Total" value={Number(s.total ?? 0)} />
        <KpiTile label="Activos" value={Number(s.active ?? 0)} tone="neutral" />
        <KpiTile
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <FilterChip active={filter === 'active'} onClick={() => setFilter('active')} label={`Activos (${items.filter(p => p.isActive).length})`} />
        <FilterChip active={filter === 'inactive'} onClick={() => setFilter('inactive')} label={`Inactivos (${items.filter(p => !p.isActive).length})`} />
        <FilterChip active={filter === 'outdated'} onClick={() => setFilter('outdated')} label={`Desact. (${items.filter(p => p.hasUpdate).length})`} />
        <FilterChip
          active={filter === 'vulnerable'}
          onClick={() => setFilter('vulnerable')}
          label={`Vulnerables (${items.filter(p => p.vulnerabilities.length > 0).length})`}
          tone="danger"
        />
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-[var(--border-default)]">
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-[var(--border-default)]">
            {filtered.length === 0 ? (
              <tr><td colSpan={7} className="py-6 text-center text-[var(--text-tertiary)]">Sin plugins que coincidan con el filtro.</td></tr>
            ) : filtered.map((p) => (
              <PluginRow
                key={p.slug}
                p={p}
      ...
      {(report.plugins.muPlugins.length > 0 || report.plugins.dropins.length > 0) && (
        <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3">
          <p className="mb-2 text-xs font-semibold text-amber-900">
            MU-plugins y drop-ins ({report.plugins.muPlugins.length + report.plugins.dropins.length})
          </p>
          <p className="mb-2 text-[11px] text-amber-900/80">
            Se cargan sin aparecer en la lista de plugins — revisar uno a uno (malware, backups, ManageWP, object-cache.php...).
          </p>
          <ul className="space-y-1 text-[11px]">
            {report.plugins.muPlugins.map((m, i) => (
              <li key={`mu-${i}`} className="flex items-center gap-2">
                <span className="rounded bg-amber-100 px-1.5 py-0.5 font-mono text-[10px]">MU</span>
            ...
            {report.plugins.dropins.map((d, i) => (
              <li key={`dr-${i}`} className="flex items-center gap-2">
                <span className="rounded bg-amber-100 px-1.5 py-0.5 font-mono text-[10px]">Drop-in</span>
        ...
        <tr className="bg-[var(--bg-secondary)]">
          <td colSpan={7} className="px-4 py-3">
            ...
            {p.vulnerabilities.length > 0 && (
              <div>
                <p className="mb-2 text-[11px] font-semibold text-red-700">
                  Vulnerabilidades conocidas ({p.vulnerabilities.length})
                </p>
                <ul className="space-y-1 text-[11px]">
                    ...
                    <li key={i} className="rounded border border-red-200 bg-red-50 p-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="ml-auto text-[10px] text-red-700">
                          {v.unfixed ? 'Sin corrección' : v.fixedIn ? `Corregido en v${v.fixedIn}` : ''}
                        </span>
                      </div>
                    </li>
                  ))}
            ...
            <div className="mt-3 flex flex-wrap gap-3 text-[10px] text-[var(--text-tertiary)]">
              {p.requiresWp && <span>Requiere WP ≥ {p.requiresWp}</span>}
              {p.requiresPhp && <span>Requiere PHP ≥ {p.requiresPhp}</span>}
              {p.networkActive && <span>Network active</span>}
            </div>
          </td>
        </tr>
      )}
